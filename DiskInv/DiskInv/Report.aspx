...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceMain" Runat="Server">
             ...
             <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:disk_inventoryConnectionString %>" SelectCommand="SELECT [disk].disk_name, borrower.fname, borrower.lname, diskHasBorrower.borrowed_date, diskHasBorrower.returned_date FROM diskHasBorrower INNER JOIN [disk] ON diskHasBorrower.disk_id = [disk].disk_id INNER JOIN borrower ON diskHasBorrower.borrower_id = borrower.borrower_id"></asp:SqlDataSource>
    </asp:Content>
